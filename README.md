...
| Folder/File           | Purpose                                                      |
| --------------------- | ------------------------------------------------------------ |
| **Dataset/**          | Contains all processed data after normalization. The `metadata.csv` file lists each sample’s name, group (Progressor, Non‑Progressor or N/A), collection time and batch. Under `DeseqScaffoldData` you find full gene expression matrices (`AllGeneFeature.csv`), differentially expressed genes (`EarlyStageDEGs.csv`), investigator‑selected top‑100 genes (`ScaffoldDeseqTop100.csv` and `ScaffoldJessSignature.csv`) . |
