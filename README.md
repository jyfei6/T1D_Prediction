...
| **temp_roc/**         | Holds intermediate ROC results generated by scripts. For example, `roc_scaffold_signatures_top100_runs1000.csv` contains mean ROC values for Scaffold Signatures Top-100 over 1000 runs, whereas `roc_deg_1000.csv` contains ROC results for the DEG comparator. |
...
├── temp_roc/
│   ├── roc_deg_1000.csv
│   ├── roc_scaffold_signatures_top100_runs1000.csv
│   └── ...
...
1. **Transcriptomic analysis** (`ScaffoldTranscriptomic.ipynb`): loads gene expression matrices from `TranscriptomicData`, using `ScaffoldSignaturesTop100.csv` for Scaffold Signatures Top-100, trains SVMs (or other models) over many random splits, computes average accuracy, sensitivity, specificity and ROC curves, and writes prediction probabilities and ROC statistics.
...
- After running the notebooks, results are organised under the `result/` and `temp_roc/` directories:
  ...
  - **Transcriptomic performance (ASS):** The `result/ASSTranscriptomic/` subdirectory contains accuracy, sensitivity and specificity (ASS) metrics. `ScaffoldSignaturesTop100_ASS.csv` stores results for Scaffold Signatures Top-100, and `DEG_ASS.csv` stores results for the separate DEG comparator.
